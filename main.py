...
VALIDATION_SECOND_CHECK_DELAY_SECS = 120
CONFIG_CHANGES_SAVE_WAIT_SECS = 30
...
MIN_ACCEPTABLE_SPEED_MBITS = 2.0
...
def download_speed_valid():
    if __download_speed_valid(*test_download_speed("1/2")):
        print(f"Waiting {VALIDATION_SECOND_CHECK_DELAY_SECS}s...")
        time.sleep(VALIDATION_SECOND_CHECK_DELAY_SECS)
        if __download_speed_valid(*test_download_speed("2/2")):
...
def main():
    ...
    for config_name, config in NETWORK_CONFIGS.items():
        ...
        (result_Mbits, _) = test_download_speed("1/1")
